...
def register_user(user: schemas.UserCreate, session: Session = Depends(get_session)):
    existing_user = session.query(models.User).filter_by(email=user.email_id).first()
    if existing_user:
        raise HTTPException(status_code=400, detail="Email already registered")

    encrypted_password =get_hashed_password(user.password)

    new_user = models.User(username=user.user_name, email=user.email_id, password=encrypted_password )
...
def token_required(func):
    @wraps(func)
    def wrapper(token: str = Depends(JWTBearer()), session: Session = Depends(get_session)):
        try:
            payload = jwt.decode(token, JWT_SECRET_KEY, algorithms=[ALGORITHM])
            user_id = payload.get("sub")
            if user_id:
                token_data = session.query(models.TokenTable).filter_by(user_id=user_id, access_toke=token, status=True).first()
                if token_data:
